...
/*
 ...
 +--------------------------------------------------------------------+
 | CiviCooP Customization for De Goede Woning (DGW)                   |
 | Author       :   Klaas Koenraadt (helpdesk@civicoop.org)           |
 | Date         :   9 September 2013                                  |
 | Description  :   Add nl_NL as valid locale for street parsing      |
 |                  so warning is not shown. Actual parsing is done   |
 |                  in extension org.civicoop.parseaddressnl          |
 +--------------------------------------------------------------------+
*/
...
ORDER BY civicrm_address.is_primary DESC, civicrm_address.location_type_id DESC, address_id ASC ";
  ...
  static function parseStreetAddress($streetAddress, $locale = NULL) {
     ...
     */
	 ...
	 /*
     * Added nl_NL as supported locale to suppress warning statement
     * Actual Dutch parsing is done in extension org.civicoop.parseaddressnl
     */
    $supportedLocalesForParsing = array('en_US', 'en_CA', 'fr_CA', 'nl_NL');
    if (!$locale) {
